...
def print_arguments(args):
    arg_descriptions = {
        'Iteration': 'The total number of training iterations',
        'batch_real': 'Batch size for processing real data',
        'batch_real_shrinkage': 'Allow shrinkage of args.batch_real if clients do not have enough images to use its initial value',
        'batch_train': 'Batch size for training models',
        'data_path': 'Path to the dataset',
        'dataset': 'The name of the dataset (e.g., CIFAR10)',
        'delta': 'Delta hyperparameter for gradient clipping',
        ...
        'dis_metric': 'The distance metric used for evaluation',
        'dsa_strategy': 'Differentiable Siamese augmentation strategy, if any',
        'epoch_eval_train': 'Number of epochs to train the model with synthetic data for evaluation',
        'eval_frequency': 'Synthetic images\'s training iterations between evaluations',
        'eval_mode': 'Evaluation mode (S: single architecture, M: multi-architecture, etc.)',
        'init': 'Initialization method for synthetic images (\'noise\' or \'real\')',
        'ipc': 'Images per class to generate for the synthetic dataset',
        'lambda_': 'Lambda hyperparameter for laplacian noise added to clipped gradients',
        'lr_img': 'Learning rate for updating synthetic images',
        'lr_net': 'Learning rate for updating network parameters',
        'method': 'Method used for training (DC: Direct Comparison, DSA: Differentiable Siamese Augmentation)',
        'model': 'The model architecture (e.g., ConvNet)',
        'model_sharing': 'Initialize the attacker\'s model with the weights of the real client\'s model',
        'num_attack_iterations': 'Number of attack iterations, or None for dataset distillation only',
        'num_clients': 'Number of clients for federated learning',
        'num_eval': 'The number of models to evaluate in the evaluation pool',
        'num_exp': 'The number of experiments to run',
        'save_path': 'Path to save the results and outputs',
...
def prepare_data(args):
    ...
    model_eval_pool = get_eval_pool(args.eval_mode, args.model, args.model)
    eval_it_pool = list(range(0, args.Iteration + 1, args.eval_frequency)) if args.eval_mode in ('S', 'SS') else [args.Iteration]
    return channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader, model_eval_pool, eval_it_pool


def split_dataset_for_clients(images_all, labels_all, indices_class, num_classes, num_clients, device):
    client_datasets = [None] * num_clients
    client_class_counts = []

    split_indices_class = []
    for class_idx in range(num_classes):
        split_indices_class.append(torch.tensor_split(torch.tensor(indices_class[class_idx], device=device), num_clients))

    for client_idx in range(num_clients):
        client_class_counts.append([0] * num_classes)

        for class_idx in range(num_classes):
            current_split_indices_class = split_indices_class[class_idx][client_idx]
            if client_datasets[client_idx] is None:
                client_datasets[client_idx] = [
                    [images_all[current_split_indices_class]],
                    [labels_all[current_split_indices_class]]
                ...
                client_datasets[client_idx][0].append(images_all[current_split_indices_class])
                client_datasets[client_idx][1].append(labels_all[current_split_indices_class])

            client_class_counts[client_idx][class_idx] += len(current_split_indices_class)

        client_datasets[client_idx][0] = torch.cat(client_datasets[client_idx][0])
        client_datasets[client_idx][1] = torch.cat(client_datasets[client_idx][1])

        idx_shuffle = torch.randperm(client_datasets[client_idx][0].shape[0])
        client_datasets[client_idx][0] = client_datasets[client_idx][0][idx_shuffle]
        client_datasets[client_idx][1] = client_datasets[client_idx][1][idx_shuffle]

    return client_datasets, client_class_counts


def initialize_syn_dataset(num_classes, ipc, channel, im_size, device):
    syn_images = torch.randn((num_classes * ipc, channel, *im_size), dtype=torch.float, requires_grad=False, device=device)
    syn_labels = torch.tensor(np.array([np.ones(ipc) * class_ for class_ in range(num_classes)]), dtype=torch.long, requires_grad=False, device=device).view(-1)
    return syn_images, syn_labels


def evaluate_syn_dataset(model_eval_pool, args, channel, num_classes, im_size, image_syn, label_syn, testloader):
    model_eval_pool_accs = []
    for model_eval in model_eval_pool:
        print(f'Evaluation model: {model_eval}')
        if args.dsa:
            args.epoch_eval_train = 10  # 1000
            args.dc_aug_param = None
            print(f'    DSA augmentation strategy: {args.dsa_strategy}')
            print(f'    DSA augmentation parameters: {args.dsa_param.__dict__}')
        else:
        ...
        for it_eval in range(args.num_eval):
            ...
            eval_syn_labels = copy.deepcopy(label_syn.detach())
            print('    ', end='')
            _, _, acc_test = evaluate_synset(it_eval, net_eval, eval_syn_images, eval_syn_labels, testloader, args)
            accs.append(acc_test)
        print(f'    Evaluating with {len(accs)} random {model_eval}:')
        print(f'        mean: {(np.mean(accs) * 100):.4f}%')
        print(f'        std: {(np.std(accs) * 100):.4f}%')
...
def save_training_results(args, experiment, iteration, image_syn, channel, std, mean):
    save_name = os.path.join(
        args.save_path, f'vis_{args.method}_{args.dataset}_{args.model}_{args.ipc}ipc_exp{experiment}_iter{iteration}.png'
    )
    image_syn_vis = copy.deepcopy(image_syn.detach().cpu())
    for channel_idx in range(channel):
        image_syn_vis[:, channel_idx] = image_syn_vis[:, channel_idx] * std[channel_idx] + mean[channel_idx]
    image_syn_vis[image_syn_vis < 0] = 0.0
    image_syn_vis[image_syn_vis > 1] = 1.0
    save_image(image_syn_vis, save_name, nrow=args.ipc)
...
def main():
    ...
    print()
    # Prepare Doorping-specific parameters if the doorping attack is enabled
    doorping_clients = []
    if args.doorping:
        doorping_perm = np.random.permutation(len(dst_train))[0: int(len(dst_train) * args.portion)]
        # Randomly select a portion of clients for Doorping attack
        doorping_clients = random.sample(range(args.num_clients), k=int(args.num_clients * args.portion))
        print(f"Clients involved in Doorping attack: {doorping_clients}")

        # Setup for Doorping-specific trigger initialization
        ...
        args.init_trigger = transform(args.init_trigger).unsqueeze(0).to(args.device, non_blocking=True)
    ...
    for experiment in range(1, args.num_exp + 1):
        ...
        client_datasets, client_class_counts = split_dataset_for_clients(
            images_all, labels_all, indices_class, num_classes, args.num_clients, args.device
        )
        print_client_details(client_datasets, client_class_counts)

        # Assign the dataset to clients
        for client_idx, client in enumerate(clients):
            client.set_real_dataset(*client_datasets[client_idx])

        image_syn, label_syn = initialize_syn_dataset(num_classes, args.ipc, channel, im_size, args.device)
        ...
        if args.init == 'real':
            ...
            for class_ in range(num_classes):
                image_syn.data[(class_ * args.ipc):((class_ + 1) * args.ipc)] = get_images(class_, args.ipc).detach().clone()
        else:
        ...
        for iteration in range(args.Iteration + 1):
            if iteration in eval_it_pool:
                ...
                for model_eval_idx, model_eval in enumerate(model_eval_pool):
                    accs_all_exps[model_eval][-1].append(model_eval_pool_accs[model_eval_idx])

                save_training_results(args, experiment, iteration, image_syn, channel, std, mean)

                if len(model_eval_pool) == 1:
                    accs = np.mean(accs_all_exps[model_eval_pool[0]][-1], axis=1)
            ...
            # Training loop for outer iteration
            for outer_iteration in range(args.outer_loop):
                ...
                for client_idx, client in enumerate(clients):
                    ...
                    # Check if the client is involved in Doorping attack
                    if client_idx in doorping_clients:
                        ...
                        # Apply the Doorping trigger to this client's images
                ...
                new_image_syn = torch.zeros_like(image_syn, dtype=torch.float, requires_grad=False, device=args.device)
                for client in clients:
            ...
            if (iteration + 1) % 10 == 0:
                print(f'{get_time()}: End of iteration #{iteration + 1}, loss is {match_loss_avg:.6f}')

    print('========== Final results ==========')
    for model_eval in model_eval_pool:
        accs = [accs_all_exps[model_eval][i][-1] for i in range(args.num_exp)]
        print(f'On {args.num_exp} experiments, when training with {args.model} and evaluating with {np.size(accs)} random {model_eval}:')
        print(f'    mean: {(np.mean(accs) * 100):.4f}%')
        print(f'    std: {(np.std(accs) * 100):.4f}%')
